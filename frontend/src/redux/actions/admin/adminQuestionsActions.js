...
const adminQuestionActions = {
  ...
  createQuestion: (newQuestion) => {
    return async (dispatch) => {
      let response = await axios.post('http://localhost:4000/api/admin/question/' + newQuestion.correctAnswer, newQuestion)
  ...
  updateQuestion: (editedQuestion) => {
    return async () => {
      let response = await axios.put('http://localhost:4000/question/id', editedQuestion)
    }
  },

  deleteQuestion: (id) => {
    return async () => {
      let response = await axios.delete('http://localhost:4000/api/admin/question/' + id)
      return response.data
    }
  }
};
